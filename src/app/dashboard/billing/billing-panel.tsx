...
import { createCheckoutSessionAction, createPortalLinkAction } from './actions';
...
export function BillingPanel({
  ...
  async function handleChoosePlan(plan: PlanOption) {
    ...
    try {
      const result = await createCheckoutSessionAction(tenantId, plan.id as 'starter' | 'pro');
      if (result.url) {
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        {plans.map((plan) => {
          ...
          return (
            <Card key={plan.id} className={cn('relative', isCurrent && 'ring-2 ring-primary')}>
              ...
              <CardContent className="space-y-3">
                ...
                {plan.id !== 'free' && !isCurrent && (
                  ...
                  >
                    {pendingPlan === plan.id ? 'Redirecting…' : `Choose ${plan.name}`}
                  </Button>
                )}
              </CardContent>
            </Card>
